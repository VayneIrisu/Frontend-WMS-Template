...
export default function CustomersPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (editItem) { await api.put(`/customers/${editItem.id}`, form); addToast('Customer diupdate!', 'success'); }
      else { await api.post('/customers', form); addToast('Customer ditambahkan!', 'success'); }
  ...
  const handleDelete = async (item) => {
    if (!confirm(`Hapus customer "${item.name}"?`)) return;
    try { await api.delete(`/customers/${item.id}`); addToast('Customer dihapus.', 'success'); loadData(); }
    catch (err) { addToast(err.message, 'error'); }
  };
  ...
  return (
    <>
      <Header title="👥 Customer" subtitle="Kelola data customer" />
      <div className="toolbar">
        <div className="toolbar-left"><div className="search-input"><span className="search-icon">🔍</span><input placeholder="Cari customer..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} /></div></div>
        <div className="toolbar-right"><button className="btn btn-primary" onClick={openAdd}>+ Tambah Customer</button></div>
      </div>
      <div className="card"><DataTable columns={columns} data={data} loading={loading} /><Pagination pagination={pagination} onPageChange={setPage} /></div>
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Customer' : 'Tambah Customer'}
